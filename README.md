...
My Color Recipeは、ユーザーが好きな写真から明るさ、コントラスト、彩度の傾向を数値化し、加工対象写真との差に基づいて加工レシピを提案します。
...
開発では、OpenAIのChatGPT Codex 5.6 Solを利用しました。AIには、機能の分け方やファイル構成、関数ごとのコード案、型ヒントとdocstring、正常値・境界値・異常値を確認するテスト案、画面構成の案を出してもらいました。また、エラーが発生したときは、エラーログと該当部分を提示し、原因と修正方法の整理に利用しました。

一方で、作る機能と実装の優先順位、6種類の加工候補の採用、写真の選択、加工結果の評価、画面の見やすさ、提案を実際に採用するかどうかは自分で判断しました。コードは一度に完成させず、「1機能とそれに対応するテスト」を一単位として段階的に実装しました。

AIが提案したコードはそのまま正しいとは考えず、pytest、Ruff、GitHub Actionsを実行して確認しました。さらに、Streamlitを実際に操作し、画像のアップロードから候補の選択、加工結果とレシピの保存までを確認しました。テストが通った場合でも、Before／Afterを自分の目で比較し、写真が好みに近づいているかを最終的な採用基準としました。
